...
一个手语视觉反查工具：拍下自己做手语动作的**多张关键帧照片**，从《国家通用手语词典》6699 个词条里找出最匹配的候选词条（命令行默认 top-5，本地网页展示 top-9，含词典插图、中文词、打法描述、置信度与理由）。

> 代码目录 `reverse-lookup/`；数据库 `sign-language-database/`（v2 增强版见 `sign-language-database-v2/`）。

## 思路

用户照片是「真人彩照」（可能单手或双手），词典图是「黑白线描半身像＋运动箭头」，画风与信息维度差异大，**不走图对图像素匹配**。改由多模态大模型换赛道：

1. **理解**：GPT-5.5 看关键帧 → 用词典词汇输出结构化描述（手型/朝向/位置/运动）。
2. **召回**：把描述渲染成贴库措辞的一句话 → 本地 BGE 中文向量 → 余弦检索 6699 条，并结合用户上传帧数与词条预计帧数做软加权 → top-40。
3. **重排**：先文本重排保留候选池（命令行默认 10；网页为 top-9 额外保留 16），再把原始帧＋候选线描图分批喂给模型视觉重排；单批候选图默认最多 5 张，以降低网关空返回概率。
...
> **多帧动作优先级**：索引构建会根据 v2 词条的 `llm_struct.steps/movement/location` 派生 `estimated_frames`、`motion_level`、`motion_tags`。查询时不会硬过滤候选，而是在向量相似度上叠加 `frame_score` 与 `motion_score`：单帧略偏向静态词条，多帧略偏向路径动作/多步动作，且会奖励“下巴到腰侧”等动作标签对齐，最终日志和报告会同时保留原始 `recall_sim` 与综合 `final_recall_score`。
...
常用参数：`--top-k`（命令行默认 5）、`--top-recall`（默认 40）、`--no-visual-rerank`、`--max-rerank-images`（默认 5）、`--json-only`、`--open`、`--backend local|api`。

每次查询都会在 `reverse-lookup/query_logs/` 下创建独立目录，保存本次输入帧副本、参数、召回候选、文本/视觉重排步骤、GPT 原始返回、usage、最终结果和异常堆栈（如有）。网页端和命令行共用同一套日志；命令行普通模式会打印 `查询日志：...`，`--json-only` 会在 JSON 中返回 `log_id` / `log_dir`。
...
| `motion_metadata.py` | 从词条描述/llm_struct 派生预计帧数与动作类型，并提供召回软加权 |
| `query.py` | 在线主流程 ＋ HTML 报告 |
| `query_logger.py` | 每次查询的本地完整记录（`query_logs/<log_id>/record.json` + 输入帧副本） |
...
- 测试题能产出可读 HTML 报告（左查询帧／右候选线描图＋词＋打法＋置信＋理由）。
...
- `query_logs/` 会包含用户上传图片、GPT 原始返回和错误堆栈，已加入 `.gitignore`，不要提交到公开仓库。
